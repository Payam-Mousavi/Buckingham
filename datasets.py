...
def sample_re(num_samples, Re_range, seed=None):
    if seed is not None:
        np.random.seed(seed)
    Re_log_vec = np.random.uniform(np.log10(Re_range[0]), np.log10(Re_range[1]), num_samples)
    # Re_vec = Re_log_vec
    Re_vec = np.power(10, Re_log_vec)
    # Re_vec = np.random.uniform(Re_range[0], Re_range[1], num_samples)
    CD, _ = run_experiments(Re_vec=Re_vec, velocities=None, diameters=None, densities=None, viscosities=None)
    return Re_vec, CD
...
class RandomDataset_scaled(Dataset):
    def __init__(self, num_samples=100000,
                 ...
                 D_range=[0.05, 0.5],
                 U_range=[0.1, 20],
                 sampling_method="naive",  # OR "unifrom-Re"
                 seed=None):
        
        np.random.seed(seed)
        if sampling_method == "naive":
            self.rho, self.mu, self.D, self.U, self.Re, self.CD = sample_parameters_naive(num_samples=num_samples,
                                                                                          Re_range=Re_range,
                                                                                          rho_range=rho_range,
                                                                                          mu_range=mu_range,
                                                                                          D_range=D_range,
